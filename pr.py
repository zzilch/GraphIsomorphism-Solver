...
def is_isomorphic_pr(G1,G2, node_match=None, edge_match=None):
    return PRMatcher(G1,G2, node_match=node_match, edge_match=edge_match).is_isomorphic()

class PRMatcher(object):
    ...
    def candidate_pairs_iter(self):
        ...
        # 取不在子图中的候选结点
        T1_inout = list(set(self.inout_1)-set(self.core_1))
        T2_inout = list(set(self.inout_2)-set(self.core_2))

        # 如果存在这样的候选结点则按后一个图的顶点顺序生成顶点对
        if T1_inout and T2_inout:
            node_2 = max(T2_inout, key=min_key)
    ...
    def syntactic_feasibility(self, G1_node, G2_node):
        ...
        # 候选连接数检查
        num1 = len(self.G1_adj_set[G1_node]&(set(self.inout_1)-set(self.core_1)))
        num2 = len(self.G2_adj_set[G2_node]&(set(self.inout_2)-set(self.core_2)))
        if not (num1 == num2):
            return False

        # 外部剩余数检查
        num1 = len(self.G1_adj_set[G1_node]-set(self.inout_1))
        num2 = len(self.G2_adj_set[G2_node]-set(self.inout_2))
        if not (num1 == num2):
                return False
...
class PRState(object):
    def __init__(self, GM, G1_node=None, G2_node=None):
        ...
        if G1_node is not None and G2_node is not None:
            ...
            # 更新G1候选结点
            new_nodes = GM.G1_adj_set[G1_node]-set(GM.inout_1)
            ...
            #     new_nodes.update(set(GM.G1[node])-set(GM.core_1))
            # for node in new_nodes-set(GM.inout_1):
            #     GM.inout_1[node] = self.depth

            # 更新G2候选结点
            new_nodes = GM.G2_adj_set[G2_node]-set(GM.inout_2)
            for node in new_nodes:
                GM.inout_2[node]=self.depth
            # new_nodes = set([])
            # for node in GM.core_2:
            #     new_nodes.update(set(GM.G2[node])-set(GM.core_2))
            # for node in new_nodes-set(GM.inout_2):
            #     GM.inout_2[node] = self.depth
